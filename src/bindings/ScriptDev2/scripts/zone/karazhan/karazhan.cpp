...
struct MANGOS_DLL_DECL npc_barnesAI : public npc_escortAI
{
    ...
    void Reset()
    {
        ...
        if (m_pInstance)
        {
            ...
            if(m_pInstance->GetData(DATA_OPERA_EVENT)==DONE)
            {
              if (GameObject* rDoor = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(DATA_GAMEOBJECT_STAGEDOORRIGHT)))
        ...
        if (PerformanceReady)
        {
            ...
            if (!RaidWiped)
            {
                if (WipeTimer < diff)
                {
                    ...
                    if (RaidWiped)
                    {
                        debug_log("SD2: Opera Event - Raid Wiped");
                        RaidWiped = true;
                        EnterEvadeMode();
    ...
    void PrepareEncounter()
    {
        ...
        for(; index < count; ++index)
        {
            ...
            if (Creature* pCreature = m_creature->SummonCreature(entry, PosX, SPAWN_Y, SPAWN_Z, SPAWN_O, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
            {

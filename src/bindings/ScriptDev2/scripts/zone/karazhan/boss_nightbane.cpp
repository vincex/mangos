            ...
            if (!Flying)
            {
                ...
                if (Charred_Timer < diff)
                {
                    ...
                    if(!targets.empty())
                    {
                        ...
                        Unit* target = *itr;
                        Target_Helper = m_creature->SummonCreature(17260,target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(),0,TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,30000);

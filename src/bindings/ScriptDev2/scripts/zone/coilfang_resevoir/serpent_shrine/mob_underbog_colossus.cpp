...
struct MANGOS_DLL_DECL mob_underbog_colossusAI : public ScriptedAI
{
	...
	switch (rand()%4)
		...
		case 1: // Toxic Pool
    		...
    		for (uint8 i=0;i<2;i++)
		    DoSpawnCreature(NPC_LURKER, 5, 5, 0, 0,TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
                break;
            case 3: // 5+ Colossus Rager  
                for (uint8 i=0;i<5+rand()%8;i++)
                    DoSpawnCreature(NPC_RAGER, 5, 5, 0, 0,TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
                break;
            /*case 4:

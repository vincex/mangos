...
struct MANGOS_DLL_DECL boss_hydross_the_unstableAI : public ScriptedAI
{
    ...
    void JustSummoned(Creature* pSummoned)
    {
        if (pSummoned->GetEntry() == NPC_PURE_SPAWN)
            ...
            pSummoned->ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_NATURE, true);
    ...
    void SpawnAdds()
    {
        for(uint8 i = 0; i < 4; ++i)
            DoSpawnCreature(m_bCorruptedForm ? NPC_TAINTED_SPAWN : NPC_PURE_SPAWN,
                afSpawnDiffs[i][0], afSpawnDiffs[i][1], 0.0f, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
    }

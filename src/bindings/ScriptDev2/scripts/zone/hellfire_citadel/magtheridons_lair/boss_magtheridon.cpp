...
struct MANGOS_DLL_DECL mob_abyssalAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* caster, const SpellEntry *spell)
    {
        ...
        if (spell->Id == SPELL_SOUL_TRANSFER)
        {
                debug_log("Magtheridon(abyssal), Soul Transfer Rimossa");
                m_creature->RemoveAurasDueToSpell(SPELL_SOUL_TRANSFER);
        }
...
struct MANGOS_DLL_DECL boss_magtheridonAI : public ScriptedAI
{
	...
	if (spell->Id == SPELL_SOUL_TRANSFER)
	{
		debug_log("Magtheridon, Soul Transfer Rimossa");
	   	m_creature->RemoveAurasDueToSpell(SPELL_SOUL_TRANSFER);
	}

...
struct MANGOS_DLL_DECL EmeraldDragonWhelpAI : public ScriptedAI
{
	#define FIREBALL 39054

	Unit* owner;
	uint32 Reset_Timer;

	EmeraldDragonWhelpAI(Creature *c) : ScriptedAI(c) 
	{
	...
	void UpdateAI(const uint32 diff)
	{
       ...
       //Segue il padrone se non è in combat
		...
		if (m_creature->getVictim())
		{
			if (Reset_Timer < diff)
			{
				DoCast(m_creature->getVictim(), FIREBALL);
				Reset_Timer = 3500;
			 }
...
CreatureAI* GetAI_EmeraldDragonWhelp(Creature *_Creature)
{
  	return new EmeraldDragonWhelpAI (_Creature);
}

void AddSC_EmeraldDragonWhelp()
{
    Script *newscript;

    newscript = new Script;
    newscript->Name="guardian_EmeraldDragonWhelp";
    newscript->GetAI = GetAI_EmeraldDragonWhelp;
